...
function Form_list(props) {
  ...
  return (
    <Container >
        ...
        <div>
          {playData.length===0 && <div className='py-5 my-4 d-flex justify-content-center'>
      ...
      {playData.map((data, index) => (
        <div key={index} className='col-lg-4 col-md-6 col-12 playCard '>
          <div className='p-3'>
            ...
            <div>
              {data.availableTimeSlots.map((slot, tindex) => (
                <p key={tindex}>{slot.value}</p>
              ))}
            </div>
            <h4>Sports</h4>
            <div>
              {data.sports.map((sport, sindex) => (
                <p key={sindex}>{sport.value}</p>
              ))}
            </div>
            <h4>Courts</h4>
            <div>
              {data.courts.map((court, cindex) => (
                <p key={cindex}>{court.value}</p>
              ))}
            </div>
            {/* {data?.file && <p>{data.file}</p>} */}
           <div className='d-flex'>
           <button
